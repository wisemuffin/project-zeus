...
What's trending in Australia right now, and does it align with high-opportunity fields? This analysis classifies real-time **Google Trends** trending searches against **UAC** fields of study and enriches them with opportunity gap context from the **Jobs and Skills Australia** Internet Vacancy Index.

For the interactive data explorer, see [Trending Interests](/demand/trending-interests).
...
- No historical accumulation — each refresh overwrites the previous snapshot.
- Classification gaps — brand names and indirect references are not captured by simple keyword matching. An LLM-based classifier could improve coverage.
