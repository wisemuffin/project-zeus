...
> Which fields of study have high job demand but low student interest?

**Source models:** `opportunity_gap`, `opportunity_gap_by_gender`
**Data sources:** IVI job vacancies (Jobs and Skills Australia), UAC Early Bird first preferences
...
- Vacancy data is national-level only (state-level analysis planned separately).
- Gender preference data uses abbreviated FOS names which are normalised during staging — verify any new FOS categories match correctly.
